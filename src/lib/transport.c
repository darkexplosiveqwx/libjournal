...
static int transport_send_memfd(int fd, const struct iovec *iov,
                                int iov_len, size_t total) {
...
int transport_send_one_shot(const struct iovec *iov, int iov_len) {
    ...
    if (total > JOURNAL_MAX_DGRAM) {
        int r = transport_send_memfd(fd, iov, iov_len, total);
        close(fd);
        return r;
    }
    ...
    if (r < 0 && err == EMSGSIZE) {
        int mr = transport_send_memfd(fd, iov, iov_len, total);
        close(fd);
        return mr;
    }
