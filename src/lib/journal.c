...
static int send_impl(const char *format, va_list ap, int one_shot) {
    ...
    while (f && n_fields < MAX_IOV) {
        ...
        if (encode_needs_binary(value_buf, (size_t)value_len)) {
            int idx = n_iov;
            int r = encode_binary(iov, MAX_IOV * 4, &idx,
                                  f, key_len,
                                  value_buf, (size_t)value_len,
                                  key_bufs[n_fields],
                                  le_bufs[n_fields]);
            if (r < 0) {
                util_consume_va_args(ap, f);
            ...
            int len = encode_text(field_buf, sizeof(text_fields[n_fields]),
                                  f, key_len,
                                  value_buf, (size_t)value_len);
            if (len < 0) {
                util_consume_va_args(ap, f);
...
int journal_print(int priority, const char *format, ...) {
    ...
    if (encode_needs_binary(msg, (size_t)msg_len)) {
        ...
        int len = encode_text(msg_field, sizeof(msg_field),
                              "MESSAGE", 7,
                              msg, (size_t)msg_len);
    ...
    if (prio_len < 0) return -EINVAL;

    int pfl = encode_text(prio_field, sizeof(prio_field),
                          "PRIORITY", 8,
                          prio_val, (size_t)prio_len);
    if (pfl < 0) return -EINVAL;
