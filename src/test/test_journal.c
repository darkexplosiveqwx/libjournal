...
#define ASSERT_EQ(a, b) do { \
    if ((a) != (b)) { \
        char _buf[256]; \
        snprintf(_buf, sizeof(_buf), "%s == %s: got %d, expected %d", \
                 #a, #b, (int)(a), (int)(b)); \
        FAIL(_buf); \
        return; \
    } \
...
#define ASSERT_STREQ(a, b) do { \
    if (strcmp((a), (b)) != 0) { \
        char _buf[512]; \
        snprintf(_buf, sizeof(_buf), "%s == %s: got \"%s\", expected \"%s\"", \
                 #a, #b, (a), (b)); \
        FAIL(_buf); \
        return; \
    } \
...
static void test_encode_binary(void) {
    ...
    int r = encode_binary(iov, 8, &idx, "MYFIELD", 7, "hello", 5,
                          key_buf, le_buf);
    ASSERT_EQ(r, 0);
    ASSERT_EQ(idx, 4);
...
static void test_sendv_binary(void) {
    ...
    int r = encode_binary(iov, 8, &idx, "BINARY", 6, data, 11,
                          key_buf, le_buf);
    ASSERT_EQ(r, 0);
    ASSERT_EQ(idx, 4);
...
int main(void) {
    ...
    TEST("accept valid keys");    test_validate_key_accept();
    TEST("reject invalid keys");  test_validate_key_reject();
    TEST("reject trusted keys");  test_validate_key_trusted();

    printf("\n=== encode_needs_binary ===\n");
    TEST("detect binary");        test_needs_binary();

    printf("\n=== encode_trim_trailing_whitespace ===\n");
    TEST("trim whitespace");      test_trim_whitespace();

    printf("\n=== encode_text ===\n");
    TEST("encode text fields");   test_encode_text();

    printf("\n=== encode_binary ===\n");
    TEST("encode binary fields");     test_encode_binary();
    TEST("empty binary field");       test_encode_binary_empty();
    TEST("iov too small");            test_encode_binary_too_small();

    printf("\n=== util ===\n");
    TEST("write_le64");                test_write_le64();
    TEST("count conversions");         test_count_conversions();

    printf("\n=== journal_send ===\n");
    TEST("bad keys ignored");          test_send_bad_key();
    TEST("NULL format");               test_send_empty();
    TEST("trailing whitespace");       test_send_trim_value();
    TEST("multiple fields");           test_send_multi();
    TEST("auto binary encoding");      test_send_auto_binary();

    printf("\n=== journal_print ===\n");
    TEST("empty and NULL format");     test_print_empty();

    printf("\n=== journal_sendv ===\n");
    TEST("basic sendv");               test_sendv_basic();
    TEST("binary via sendv");          test_sendv_binary();

    printf("\n=== init/close ===\n");
    TEST("init and close");            test_init_close();

    printf("\n=== one-shot ===\n");
    TEST("one-shot variants");         test_once_variants();
